...
- (void)viewDidLoad {
    ...
    self.location_manager.desiredAccuracy = kCLLocationAccuracyKilometer;
...
//Private methods
- (void)sendLocalNotification:(NSString*)body_text{
    self.notification_center = [UNUserNotificationCenter currentNotificationCenter];
    self.notification_center.delegate = self;

    UNMutableNotificationContent *content = [[UNMutableNotificationContent alloc] init];
    content.title = @"PlaceReminder";
...
-(NSMutableArray*)get_all_regions{
    ...
    for (MPLocation* location in self.location_list.locations) {
        CLCircularRegion* region =[[CLCircularRegion alloc] initWithCenter:location.coordinate_location.coordinate radius:5000 identifier:location.name];
...
-(void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region{
    NSString *body_text = [NSString stringWithFormat:@"You are entered of the zone %@", region.identifier];
    [self sendLocalNotification:body_text];
}

-(void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region{
    NSString *body_text = [NSString stringWithFormat:@"You are out of the zone %@", region.identifier];
    [self sendLocalNotification:body_text];
}
